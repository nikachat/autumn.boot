    ...
    public class EntityMaintenance
    {
        ...
        public EntityInfo GetEntityInfo(Type type)
        {
            ...
            () =>
            {
                ...
                var DbLiteAttributeInfo = type.GetTypeInfo().GetCustomAttributes(typeof(DbLiteTable), true).Where(it => it is DbLiteTable).SingleOrDefault();
                if (DbLiteAttributeInfo.HasValue())
                {
                    var DbLiteTable = (DbLiteTable)DbLiteAttributeInfo;
                    result.DbTableName = DbLiteTable.TableName;
                    result.TableDescription = DbLiteTable.TableDescription;
                }
        ...
        private void SetColumns(EntityInfo result)
        {
            foreach (var property in result.Type.GetProperties())
            {
                ...
                var DbLiteColumn = property.GetCustomAttributes(typeof(DbLiteColumn), true)
                .Where(it => it is DbLiteColumn)
                .Select(it => (DbLiteColumn)it)
                .FirstOrDefault();
                column.DbTableName = result.DbTableName;
                column.EntityName = result.EntityName;
                column.PropertyName = property.Name;
                column.PropertyInfo = property;
                if (DbLiteColumn.IsNullOrEmpty())
                {
                    column.DbColumnName = property.Name;
                }
                else
                {
                    if (DbLiteColumn.IsIgnore == false)
                    {
                        column.DbColumnName = DbLiteColumn.ColumnName.IsNullOrEmpty() ? property.Name : DbLiteColumn.ColumnName;
                        column.IsPrimarykey = DbLiteColumn.IsPrimaryKey;
                        column.IsIdentity = DbLiteColumn.IsIdentity;
                        column.ColumnDescription = DbLiteColumn.ColumnDescription;
                        column.IsNullable = DbLiteColumn.IsNullable;
                        column.Length = DbLiteColumn.Length;
                        column.OldDbColumnName = DbLiteColumn.OldColumnName;
                        column.DataType = DbLiteColumn.ColumnDataType;
                        column.DecimalDigits = DbLiteColumn.DecimalDigits;
                        column.OracleSequenceName = DbLiteColumn.OracleSequenceName;
                        column.IsOnlyIgnoreInsert = DbLiteColumn.IsOnlyIgnoreInsert;
                        column.IsEnableUpdateVersionValidation = DbLiteColumn.IsEnableUpdateVersionValidation;
                        column.IsTranscoding = DbLiteColumn.IsTranscoding;
                        column.SerializeDateTimeFormat = DbLiteColumn.SerializeDateTimeFormat;
                        column.IsJson = DbLiteColumn.IsJson;
                        column.NoSerialize = DbLiteColumn.NoSerialize;
                        column.DefaultValue = DbLiteColumn.DefaultValue;
                        column.IndexGroupNameList = DbLiteColumn.IndexGroupNameList;
                        column.IsOnlyIgnoreUpdate = DbLiteColumn.IsOnlyIgnoreUpdate;
                    }
                    else
                    {
