...
namespace SqlDbLite
{
    ...
    {
        public virtual ISqlDbLiteClient Context { get; set; }
        ...
        internal string GetClassString(DbTableInfo tableInfo, ref string className)
        {
            ...
            classText = classText.Replace(DbFirstTemplate.KeyDbLiteTable, IsAttribute ? string.Format(DbFirstTemplate.ValueDbLiteTable, tableInfo.Name) : null);
            if (columns.HasValue())
            {
        ...
        internal string GetClassString(List<DbColumnInfo> columns, ref string className)
        {
            ...
            classText = classText.Replace(DbFirstTemplate.KeyDbLiteTable, IsAttribute ? string.Format(DbFirstTemplate.ValueDbLiteTable, className) : null);
            if (columns.HasValue())
            {
                foreach (var item in columns)
                {
        ...
        {
            string DbLiteColumnText = DbFirstTemplate.ValueDbLiteCoulmn;
            var propertyName = GetPropertyName(item);
            var isMappingColumn = propertyName != item.DbColumnName;
            var hasDbLiteColumn = item.IsPrimarykey == true || item.IsIdentity == true || isMappingColumn;
            if (hasDbLiteColumn && this.IsAttribute)
            {
                ...
                DbLiteColumnText = string.Format(DbLiteColumnText, string.Join(",", joinList));
            }
            else
            {
                DbLiteColumnText = null;
            }
            ...
            PropertyText = PropertyText.Replace(DbFirstTemplate.KeyDbLiteColumn, DbLiteColumnText);

            if (DbLiteColumnText == null)
                PropertyText = PropertyText.Remove(0, 2);
