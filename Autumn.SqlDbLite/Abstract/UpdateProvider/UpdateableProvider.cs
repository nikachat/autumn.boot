...
namespace SqlDbLite
{
    ...
    {
        #region Property
        public SqlDbLiteProvider Context { get; internal set; }
        ...
        public IUpdateable<T> Where(string fieldName, string conditionalType, object fieldValue)
        {
            ...
            this.UpdateBuilder.Parameters.Add(new DbLiteParameter(parameterName, fieldValue));
            return this;
        }
        ...
        private void PreToSql()
        {
            ...
            if (this.IsSingle)
            {
                foreach (var item in this.UpdateBuilder.DbColumnInfoList)
                {
                    if (this.UpdateBuilder.Parameters == null) this.UpdateBuilder.Parameters = new List<DbLiteParameter>();
                    if (this.UpdateBuilder.SetValues.Any(it => this.SqlBuilder.GetNoTranslationColumnName(it.Key) == item.PropertyName))
                    {
                        continue;
                    }
                    this.UpdateBuilder.Parameters.Add(new DbLiteParameter(this.SqlBuilder.SqlParameterKeyWord + item.DbColumnName, item.Value, item.PropertyType));
                }
            }
        ...
        private void After(string sql)
        {
            if (this.IsEnableDiffLogEvent)
            {
                ...
                if (parameters == null)
                    parameters = new List<DbLiteParameter>();
        ...
        private List<DiffLogTableInfo> GetDiffTable(string sql, List<DbLiteParameter> parameters)
        {
